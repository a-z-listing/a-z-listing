jQuery( function( $ ) {
    const wireup_a_z_listing = function() {
        jQuery( '.a-z-listing-widget' ).each( function( idx, el ) {
            ...
            const target_post_title                = el.find('.a-z-listing-target-post-title');
            const display_type                     = el.find('.a-z-listing-display-type');
            const listing_post_type                = el.find('.a-z-listing-post-type');
            const listing_post_type_wrapper        = el.find('.a-z-listing-post-type-wrapper');
            const listing_parent_post              = el.find('.a-z-listing-parent-post');
            const listing_parent_post_title        = el.find('.a-z-listing-parent-post-title');
            const listing_parent_post_wrapper      = el.find('.a-z-listing-parent-post-wrapper');
            const listing_taxonomy                 = el.find('.a-z-listing-taxonomy');
            const listing_taxonomy_wrapper         = el.find('.a-z-listing-taxonomy-wrapper');
            const listing_parent_term              = el.find('.a-z-listing-parent-term');
            const listing_parent_term_wrapper      = el.find('.a-z-listing-parent-term-wrapper');
            ...
            $( target_post_title ).autocomplete( {
                source: function( post_title, response ) {
                    jQuery.ajax( {
                        url:      a_z_listing_widget_admin.ajax_url || '/wp-admin/admin-ajax.php',
                        type:     'POST',
                        dataType: 'json',
                        data: {
    ...
    wireup_a_z_listing();
    $( document ).on( 'widget-updated widget-added', function( e ) { wireup_a_z_listing(); } );
} );
