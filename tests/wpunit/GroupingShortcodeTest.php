...
class AZ_Shortcode_Grouping_Tests extends \Codeception\TestCase\WPTestCase {
    /**
     * @var \UnitTester
     */
    protected $tester;
	...
	public function testEmpty() {
		$expected = file_get_contents( 'tests/_data/default-listing-grouped.txt' );
	...
	public function testPopulatedListing() {
		...
		$expected = sprintf( file_get_contents( 'tests/_data/populated-listing-grouped.txt' ), $title, $url );
		$actual   = do_shortcode( '[a-z-listing grouping="3"]' );
	...
	public function testPopulatedTaxonomyListing() {
		...
		$expected = sprintf( file_get_contents( 'tests/_data/populated-taxonomy-listing-grouped.txt' ), $title, $url );
